...
namespace uva
{
    namespace lang
    {
        ...
        {
        public:
            source_cursor(std::string_view source);
            source_cursor() = default;
        public:
            ...
            const source_position& start() const {
                return m_start;
            }
            const source_position& end() const {
                return m_end;
            }
        ...
        private:
            ...
            source_position m_start;
            source_position m_end;

            std::vector<source_cursor> m_children;

            cursor_type m_type = cursor_type::cursor_undefined;
        //parsing
        protected:
            void parse();
            source_cursor init_next();
            const char& discard();
            const char& extend();
            ...
            void update_position(source_position& position, const char& token);
            ...
            void extend_by(const source_cursor& cursor);
            void extend_by_last_child_if_exists();
        //parsing
        public:
            source_cursor parse_next();

            const source_cursor& child_from_type(const cursor_type& __type) const {
            ...
            const source_cursor& block() const {
                return child_from_type(cursor_type::cursor_block);
            }

            const source_cursor& fncallparams() const {
                return child_from_type(cursor_type::cursor_fncallparams);
            }

            const std::vector<source_cursor>& children() const {
                return m_children;
            }
        };
    }
